...
function secao(int $num, string $titulo): void
{
    ...
    echo "\n" . c('bold', 'vermelho') . "  ► [$num] $titulo\n  $sep\n" . rst();
}

function cabecalho(string $titulo): void
{
    echo "\n" . c('bold', 'branco') . "  $titulo\n  " . str_repeat('=', mb_strlen($titulo)) . "\n\n" . rst();
}

function inputUsuario(string $mensagem): void
{
    echo c('rst', 'bold', 'vermelho') . "  ▸ $mensagem: " . c('fverde');
}

// BANNER EXCLUSIVO - FN SS
function fnBanner(): void
{
    echo c('vermelho') . "
  " . c('bold', 'branco') . "SISTEMA DE ANÁLISE AVANÇADA - ANDROID" . c('vermelho') . "
  " . c('cinza') . "Desenvolvido por: " . c('bold', 'branco') . "DONO FN" . c('vermelho') . "
  " . c('cinza') . "Suporte: (21) 96717-3601 | Discord: @donofn" . c('vermelho') . "
  ...
  " . c('branco') . "STATUS: " . c('fverde') . "ANALISTA SS ATIVO" . rst() . "\n\n";
}
...
{
    echo "\n" . c('bold', 'vermelho') . "  ┌─ FN SS UPDATER\n" . rst();
    echo c('vermelho') . "  ⟳ Atualizando sistema de analistas, aguarde...\n\n" . rst();
...
function detectarBypassShell(): bool
{
    ...
    echo "\n" . c('bold', 'vermelho') . "  ► RESUMO DA ANÁLISE\n  -------------------\n\n" . rst();
...
{
    echo "\n" . c('bold', 'vermelho') . "  → Checando se possui Root...\n" . rst();
    $motivos = [];
...
{
    echo "\n" . c('bold', 'vermelho') . "  → Verificando hack SSH/remoto...\n" . rst();
    $motivos = [];
...
{
    echo "\n" . c('bold', 'vermelho') . "  → Verificando scripts ativos em segundo plano...\n" . rst();
    $scripts = adb(
    ...
    echo c('bold', 'vermelho') . "  [+] Finalizando sessões bash desnecessárias...\n" . rst();
...
{
    echo "\n" . c('bold', 'vermelho') . "  → Checando se o dispositivo foi reiniciado recentemente...\n" . rst();
...
{
    echo c('bold', 'vermelho') . "  → Verificando mudanças de data/hora...\n" . rst();
    ...
    echo "\n" . c('bold', 'vermelho') . "  [+] Checando configuração automática de data/hora...\n" . rst();
...
{
    echo c('bold', 'vermelho') . "  [+] Obtendo os últimos acessos do Google Play Store...\n" . rst();
    $raw = adb('adb shell "dumpsys usagestats 2>/dev/null | grep MOVE_TO_FOREGROUND | grep com.android.vending | tail -n 5"');

    $datas = [];
    foreach (array_filter(explode("\n", $raw)) as $linha) {
        if (preg_match('/time="([^"]+)"/', $linha, $m)) {
            try {
                $dt = new DateTime($m[1]);
                $datas[] = $dt->format('d/m/Y H:i:s');
            } catch (Exception) {
                $datas[] = $m[1];
            }
        }
    ...
    if (!empty($datas)) {
        ...
        foreach ($datas as $d) echo c('amarelo') . "    $d\n" . rst();
    } else {
...
{
    echo c('bold', 'vermelho') . "  [+] Obtendo os últimos textos copiados...\n" . rst();
...
{
    echo c('bold', 'vermelho') . "  → Checando se o replay foi passado...\n" . rst();
    ...
    $batchOut = adb(
        'adb shell "stat -c \'%n|%X|%Y|%Z\' ' .
        '/sdcard/Android/data/' . $pacote . '/files/MReplays/*.bin ' .
        '/sdcard/Android/data/' . $pacote . '/files/MReplays 2>/dev/null"'
    );

    $statsMap = [];
    foreach (array_filter(explode("\n", $batchOut)) as $linha) {
        $p = explode('|', trim($linha), 4);
        if (count($p) !== 4) continue;
        $statsMap[trim($p[0])] = [(int)$p[1], (int)$p[2], (int)$p[3]];
    }

    $pastaStats = $statsMap[$mreplaysDir] ?? null;
    unset($statsMap[$mreplaysDir]);

    uasort($statsMap, fn($a, $b) => $b[1] - $a[1]);
    $arquivos = array_keys($statsMap);
    ...
    foreach ($arquivos as $idx => $arquivo) {
        [$atime, $mtime, $ctime] = $statsMap[$arquivo];

        if ($idx === 0) {
            $arquivoMaisRecente = $arquivo;
            $ultimoModifyTime   = $mtime;
    ...
    if ($pastaStats) {
        [$pAtime, $pMtime, $pCtime] = $pastaStats;
        ...
        if ($ultimoModifyTime && $pMtime < $ultimoModifyTime - 10)
    ...
    if (!empty(trim($logcatUsb))) {
        ...
        foreach (array_filter(explode("\n", $logcatUsb)) as $linhaUsb) {
            if (preg_match('/^(\d{2}-\d{2}) (\d{2}:\d{2}:\d{2})/', $linhaUsb, $mUsb)) {
                ...
                $dataUsb = implode('-', array_reverse(explode('-', $mUsb[1]))); 
                if ($usbTs !== false && ($ultimoModifyTime === null || abs($usbTs - $ultimoModifyTime) <= 3600)) {
                    if (str_contains($linhaUsb, 'connected=true')) {
                        $motivos[] = "Motivo 15 - Dispositivo externo conectado via USB às $dataUsb {$mUsb[2]}";
                    } elseif (str_contains($linhaUsb, 'UsbModeChooser') || str_contains($linhaUsb, 'UsbDetails')) {
                        $motivos[] = "Motivo 15 - Configuração de conexão USB alterada às $dataUsb {$mUsb[2]}";
                    }
                }
            }
        }
    ...
    if ($pastaStats) {
        $dtAcesso = (new DateTime())->setTimestamp($pastaStats[0]);
...
{
    echo c('bold', 'vermelho') . "  → Checando bypass de Wallhack/Holograma...\n" . rst();
    ...
    echo c('bold', 'vermelho') . "  → Verificando arquivos específicos...\n" . rst();
    ...
    foreach ($pastasEspecificas as $pasta) {
        ...
        if (empty($lista)) {
            echo c('amarelo') . "  [*] Sem itens baixados! Verifique se a data é após o fim da partida!\n\n" . rst();
            continue;
        }
...
{
    echo c('bold', 'vermelho') . "  → Checando OBB...\n" . rst();
    ...
    if (empty($resultObb)) {
        echo c('amarelo') . "[*] OBB deletada e/ou inexistente!\n" . rst();
        return;
    }
...
function verificarShaders(string $pacote): void
{
    ...
    if (empty($resultShaders)) {
        info("Nenhuma alteração suspeita encontrada nos Shaders.");
        return;
    }
...
function escanearFreeFire(string $pacote, string $nomeJogo): void
{
    ...
    if (!empty($androidVer)) {
        echo c('bold', 'vermelho') . "  [+] Versão do Android: $androidVer\n" . rst();
    }

    verificarRoot();
    verificarScriptsAtivos();

    echo c('bold', 'vermelho') . "  → Verificando bypasses de funções shell...\n" . rst();
    detectarBypassShell();
    ...
    echo c('bold', 'branco') . "\n\n\t Scanner FN SS concluído. Obrigado por utilizar.\n";
    echo c('bold', 'vermelho') . "\t Assinado: DONO FN | Especialista SS\n\n" . rst();
}
...
function conectarADB(): void
{
    ...
    echo c('bold', 'vermelho') . "  → Verificando se o ADB está instalado...\n" . rst();
    if (empty(adb('adb version'))) {
...
{
    echo c('bold', 'vermelho') . "  ╔════════════════════════════════╗\n";
    echo c('bold', 'vermelho') . "  ║       SISTEMA FN SS ANDROID    ║\n";
    echo c('bold', 'vermelho') . "  ╚════════════════════════════════╝\n\n" . rst();
...
while (true) {
    ...
    switch ($opcao) {
        ...
        case 'S':
            echo "\n\n\t Sistema FN SS encerrado. Assinado: DONO FN.\n\n";
            exit(0);
    }
}
